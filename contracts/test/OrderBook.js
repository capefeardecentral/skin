...
describe("OrderBook", () => {
  ...
  async function deployOrderBookWithOrders() {
    ...
    await orderBook
      .connect(bob)
      .makeBid(0, { price: 10000, amount: 5 }, { value: 50000 });
    await orderBook
      .connect(alice)
      .makeBid(1, { price: 10000, amount: 5 }, { value: 50000 });
  ...
  async function checkBalance(orderBook) {
    ...
    const prizePool = await orderBook.prizePool();
    const expectedFunds = prizePool.add(escrow);
  ...
  describe("makeBid()", () => {
    it("rejects a bid if not enough funds are sent", async () => {
      ...
      await expect(
        orderBook
          .connect(bob)
          .makeBid(0, { price: 10000, amount: 5 }, { value: 40000 })
      ).to.be.reverted;
    });
    ...
    it("rejects a bid if too many funds are sent", async () => {
      ...
      await expect(
        orderBook
          .connect(bob)
          .makeBid(0, { price: 10000, amount: 5 }, { value: 60000 })
      ).to.be.reverted;
    });
    ...
    it("rejects a bid with amount 0", async () => {
      ...
      await expect(
        orderBook
          .connect(bob)
          .makeBid(0, { price: 10000, amount: 0 }, { value: 10000 })
      ).to.be.reverted;
    });
    ...
    it("rejects a bid with price 0", async () => {
      const { orderBook, bob } = await loadFixture(deployOrderBook);
      await expect(
        orderBook.connect(bob).makeBid(0, { price: 0, amount: 5 }, { value: 0 })
      ).to.be.reverted;
    });
    ...
    it("creates an initial bid", async () => {
      ...
      // check bid ledger
      const { higherPrice, lowerPrice, price, maker, amount } =
        await orderBook.bids(0, 1);
      expect(higherPrice).to.equal(0);
      expect(lowerPrice).to.equal(0);
    ...
    it("creates a bid after an initial bid is taken", async () => {
      ...
      await orderBook.connect(bob).makeAsk(0, { price: 11000, amount: 2 });
      await orderBook
        .connect(charles)
        .makeBid(0, { price: 11000, amount: 2 }, { value: 22000 });
      await orderBook
        .connect(bob)
        .makeBid(0, { price: 11000, amount: 2 }, { value: 22000 });
      ...
      expect(newBid.higherPrice).to.equal(0);
      expect(newBid.lowerPrice).to.equal(0);
    ...
    it("creates creates a bid after not finding a matching ask", async () => {
      ...
      await orderBook
        .connect(charles)
        .makeBid(0, { price: 10000, amount: 2 }, { value: 20000 });
      const charlesBid = await orderBook.bids(0, 2);

      expect(await orderBook.escrow()).to.equal(20000);
      expect(charlesBid.higherPrice).to.equal(0);
      expect(charlesBid.lowerPrice).to.equal(0);
      expect(await orderBook.bestBidId(0)).to.equal(2);
      expect(await orderBook.bidHead(0)).to.equal(3);
      await checkBalance(orderBook);
    });
    ...
    it("places a higher bid", async () => {
      ...
      await orderBook
        .connect(alice)
        .makeBid(0, { price: 11000, amount: 5 }, { value: 55000 });
      ...
      expect(bobsBid.higherPrice).to.equal(2);
      expect(bobsBid.lowerPrice).to.equal(0);
      expect(alicesBid.higherPrice).to.equal(0);
      expect(alicesBid.lowerPrice).to.equal(1);
      await checkBalance(orderBook);
    });
    ...
    it("sorts a bid between high and low", async () => {
      ...
      await orderBook
        .connect(charles)
        .makeBid(0, { price: 10500, amount: 5 }, { value: 52500 });
      ...
      expect(bobsBid.higherPrice).to.equal(3);
      expect(bobsBid.lowerPrice).to.equal(0);
      expect(alicesBid.higherPrice).to.equal(0);
      expect(alicesBid.lowerPrice).to.equal(3);
      expect(charlesBid.higherPrice).to.equal(2);
      expect(charlesBid.lowerPrice).to.equal(1);
      await checkBalance(orderBook);
    });
    ...
    it("sorts a matching bid under the existing bid", async () => {
      ...
      expect(bobsBid.higherPrice).to.equal(0);
      expect(bobsBid.lowerPrice).to.equal(2);
      expect(alicesBid.higherPrice).to.equal(1);
      expect(alicesBid.lowerPrice).to.equal(0);
      await checkBalance(orderBook);
    });
  });
  ...
  describe("match_token_pair_bids()", () => {
    it("mints a token pair for symmetric bids", async () => {
      ...
      const tokenPrice = await orderBook.tokenPrice();
      // TODO I think we should split out escrow and token pool
      expect(await orderBook.escrow()).to.equal(0);
      expect(await orderBook.prizePool()).to.equal(tokenPrice * 5);
    ...
    it("mints a token pair for a partial bid match less than", async () => {
      ...
      await orderBook
        .connect(alice)
        .makeBid(1, { price: 10000, amount: 3 }, { value: 30000 });

      // TODO I think we should split out escrow and token pool
      expect(await orderBook.escrow()).to.equal(20000);
      expect(await orderBook.prizePool()).to.equal(60000);
    ...
    it("mints a token pair for a partial bid match greater than", async () => {
      ...
      await orderBook
        .connect(alice)
        .makeBid(1, { price: 10000, amount: 8 }, { value: 80000 });
      const alicesBid = await orderBook.bids(1, 1);

      // TODO I think we should split out escrow and token pool
      expect(await orderBook.escrow()).to.equal(30000);
      expect(await orderBook.prizePool()).to.equal(100000);
      expect(await orderBook.ledger(bob.address, 0)).to.equal(5);
      expect(await orderBook.ledger(alice.address, 1)).to.equal(5);
      expect(await orderBook.bestBidId(0)).to.equal(0);
  ...
  describe("makeAsk", async () => {
    ...
    it("allows a user to place a better ask", async () => {
      const { orderBook, bob } = await loadFixture(deployOrderBookWithOrders);
      await orderBook.connect(bob).makeAsk(0, { price: 11000, amount: 3 });
      await orderBook.connect(bob).makeAsk(0, { price: 10500, amount: 2 });
      ...
      expect(lowerAsk.higherPrice).to.equal(1);
      expect(lowerAsk.lowerPrice).to.equal(0);
      expect(higherAsk.higherPrice).to.equal(0);
      expect(higherAsk.lowerPrice).to.equal(2);
      await checkBalance(orderBook);
    });
    ...
    it("allows a user to place a worse ask", async () => {
      const { orderBook, bob } = await loadFixture(deployOrderBookWithOrders);
      await orderBook.connect(bob).makeAsk(0, { price: 11000, amount: 3 });
      await orderBook.connect(bob).makeAsk(0, { price: 12500, amount: 2 });
      const higherAsk = await orderBook.asks(0, 2);
      const lowerAsk = await orderBook.asks(0, 1);

      expect(await orderBook.bestAskId(0)).to.equal(1);
      expect(lowerAsk.higherPrice).to.equal(2);
      expect(lowerAsk.lowerPrice).to.equal(0);
      expect(higherAsk.higherPrice).to.equal(0);
      expect(higherAsk.lowerPrice).to.equal(1);
      await checkBalance(orderBook);
    });

    it("sorts an ask between high and low", async () => {
      const { orderBook, bob } = await loadFixture(deployOrderBookWithOrders);
      await orderBook.connect(bob).makeAsk(0, { price: 11000, amount: 1 });
      await orderBook.connect(bob).makeAsk(0, { price: 13000, amount: 2 });
      await orderBook.connect(bob).makeAsk(0, { price: 12000, amount: 2 });
      ...
      expect(await orderBook.bestAskId(0)).to.equal(1);
      expect(lowerAsk.higherPrice).to.equal(3);
      expect(lowerAsk.lowerPrice).to.equal(0);
      expect(middleAsk.higherPrice).to.equal(2);
      expect(middleAsk.lowerPrice).to.equal(1);
      expect(higherAsk.higherPrice).to.equal(0);
      expect(higherAsk.lowerPrice).to.equal(3);
      await checkBalance(orderBook);
    });
    ...
    it("sorts an equal ask under the existing ask", async () => {
      ...
      expect(secondAsk.lowerPrice).to.equal(1);
      expect(secondAsk.higherPrice).to.equal(0);
      expect(firstAsk.lowerPrice).to.equal(0);
      expect(firstAsk.higherPrice).to.equal(2);
      await checkBalance(orderBook);
    });
    ...
    it("settles a partially matched ask greater than ask first", async () => {
      ...
      await orderBook
        .connect(charles)
        .makeBid(0, { price: 11000, amount: 4 }, { value: 44000 });
      const bobsAsk = await orderBook.asks(0, 1);
    ...
    it("successfully places an ask after the only ask is settled", async () => {
      ...
      expect(newAsk.higherPrice).to.equal(0);
      expect(newAsk.lowerPrice).to.equal(0);
      expect(await orderBook.bestAskId(0)).to.equal(2);
  ...
  describe("cancelAsk", () => {
    it("reverts if the ask does not exist", async () => {
      const { orderBook, bob } = await loadFixture(deployOrderBookWithOrders);
      await expect(orderBook.connect(bob).cancelAsk(0, 5)).to.be.reverted;
    });

    it("allows a user to cancel their own ask", async () => {
      const { orderBook, bob } = await loadFixture(deployOrderBookWithOrders);
      await orderBook.connect(bob).makeAsk(0, { price: 11000, amount: 2 });
      await orderBook.connect(bob).cancelAsk(0, 1);
    ...
    it("does not allow a user to cancel anothers ask", async () => {
      ...
      await orderBook.connect(bob).makeAsk(0, { price: 11000, amount: 2 });
      await expect(orderBook.connect(charles).cancelAsk(0, 1)).to.be.reverted;
    });
  });
  ...
  describe("cancelBid", () => {
    it("reverts if the bid does not exist", async () => {
      const { orderBook, bob } = await loadFixture(deployOrderBookWithOrders);
      await expect(orderBook.connect(bob).cancelBid(0, 5)).to.be.reverted;
      await checkBalance(orderBook);
    });
    ...
    it("allows a user to cancel their own bid", async () => {
      ...
      await orderBook
        .connect(bob)
        .makeBid(0, { price: 11000, amount: 2 }, { value: 22000 });
      await orderBook.connect(bob).cancelBid(0, bidHead);
    ...
    it("does not allow a user to cancel anothers bid", async () => {
      ...
      await orderBook
        .connect(bob)
        .makeBid(0, { price: 11000, amount: 2 }, { value: 22000 });
      await expect(orderBook.connect(charles).cancelBid(0, bidHead)).to.be
        .reverted;
    });
  });
  ...
  describe("takeAsk", () => {
    it("reverts if the ask does not exist", async () => {
      const { orderBook, bob } = await loadFixture(deployOrderBookWithOrders);
      await expect(orderBook.connect(bob).takeAsk(0, 5)).to.be.reverted;
    });
    ...
    it("reverts if the user does not send enough value", async () => {
      ...
      await orderBook.connect(bob).makeAsk(0, { price: 11000, amount: 2 });
      await expect(
        orderBook.connect(charles).takeAsk(0, askHead, { value: 10000 })
      ).to.be.reverted;
    });
    ...
    it("allows a user to take an ask", async () => {
      ...
      const askHead = await orderBook.askHead(0);
      await orderBook.connect(bob).makeAsk(0, { price: 11000, amount: 2 });
      await orderBook.connect(charles).takeAsk(0, askHead, { value: 22000 });
    ...
    it("updates the best ask when the best ask is taken", async () => {
      ...
      expect(newBestAsk.higherPrice).to.equal(0);
      expect(newBestAsk.lowerPrice).to.equal(0);
      await checkBalance(orderBook);
    });
    ...
    it("resorts the orderbook when the best ask is taken", async () => {
      ...
      await orderBook.connect(bob).makeAsk(0, { price: 11000, amount: 1 });
      await orderBook.connect(bob).makeAsk(0, { price: 12000, amount: 2 });
      await orderBook.connect(bob).makeAsk(0, { price: 13000, amount: 1 });
      await orderBook
        .connect(charles)
        .takeAsk(0, askHead.add(1), { value: 24000 });
      ...
      expect(firstAsk.higherPrice).to.equal(askHead.add(2));
      expect(firstAsk.lowerPrice).to.equal(0);
      expect(thirdAsk.higherPrice).to.equal(0);
      expect(thirdAsk.lowerPrice).to.equal(askHead);
      await checkBalance(orderBook);
    });
  });
  ...
  describe("takeBid", () => {
    it("reverts if the bid does not exist", async () => {
      const { orderBook, bob } = await loadFixture(deployOrderBookWithOrders);
      await expect(orderBook.connect(bob).takeBid(0, 5)).to.be.revertedWith(
        "bid does not exist"
      );
    });
    ...
    it("reverts if user doesn't have enough tokens to fill the bid", async () => {
      ...
      await orderBook
        .connect(charles)
        .makeBid(0, { price: 10000, amount: 10 }, { value: 100000 });
      await expect(
        orderBook.connect(bob).takeBid(0, bidHead)
    ...
    it("allows a user to take a bid", async () => {
      ...
      await orderBook
        .connect(charles)
        .makeBid(0, { price: 10000, amount: 2 }, { value: 20000 });
      await orderBook.connect(bob).takeBid(0, bidHead);
      expect(await orderBook.ledger(bob.address, 0)).to.equal(3);
      expect(await orderBook.ledger(charles.address, 0)).to.equal(2);
      await checkBalance(orderBook);
    });
    ...
    it("updates the best bid when the best bid is taken", async () => {
      ...
      await orderBook
        .connect(charles)
        .makeBid(0, { price: 11000, amount: 1 }, { value: 11000 });
      await orderBook.connect(bob).takeBid(0, bidHead);
      expect(await orderBook.ledger(bob.address, 0)).to.equal(3);
      expect(await orderBook.ledger(charles.address, 0)).to.equal(2);
      const newBestBid = await orderBook.bids(0, bidHead.add(1));
      expect(await orderBook.bestBidId(0)).to.equal(bidHead.add(1));
      expect(newBestBid.higherPrice).to.equal(0);
      expect(newBestBid.lowerPrice).to.equal(0);
      await checkBalance(orderBook);
    });
    ...
    it("resorts the orderbook when a bid is taken", async () => {
      ...
      await orderBook
        .connect(charles)
        .makeBid(0, { price: 12000, amount: 1 }, { value: 12000 });
      await orderBook.connect(bob).takeBid(0, secondBid);
      expect(await orderBook.ledger(bob.address, 0)).to.equal(4);
      expect(await orderBook.ledger(charles.address, 0)).to.equal(1);
      ...
      expect(thirdBidBody.higherPrice).to.equal(0);
      expect(thirdBidBody.lowerPrice).to.equal(bidHead);
      expect(bidHeadBody.higherPrice).to.equal(thirdBid);
      expect(bidHeadBody.lowerPrice).to.equal(0);
      await checkBalance(orderBook);
    });
  });
});
