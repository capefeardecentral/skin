...
contract Consensus is OrderBook {
    ...
    function settle() external {
        ...
        require(consensus_winner == Tokens.NONE, "Consensus: vote is already settled");
        ...
        if (consensusReached) {
            consensus_winner = consensusResult == Tokens.YES ? Tokens.YES : Tokens.NO;
            reward = consensus_winner == Tokens.YES ? _get_reward(yesVotes, noVotes) : _get_reward(noVotes, yesVotes);
        } else {
            // fail with error
    ...
    function claim() external {
        require(block.timestamp >= voteEndTime, "Consensus: Voting has not ended");
        require(consensus_winner != Tokens.NONE, "Consensus: Consensus has not been reached");
